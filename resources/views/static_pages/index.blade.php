@extends('layouts.default')
@section('content')
    <!-- 精品项目 -->
    <section class="itemBoxs wPub">
        <div class="itemTit clearfix">
            <h3 class="fl">精品项目</h3>
        </div>
        <div class="itemLists clearfix">
            <div class="itemlist boxIndex">
                <a href="#" target="_blank" class="iBlock relative">
                    <figure class="relative">
                        <img src="" alt="" />
                    </figure>
                    <em class="iBlock absolute fourIcon Hot"></em>
                    <p style="text-align: center;font-size: 14px;">美白嫩肤</p>
                </a>
            </div>
            <div class="itemlist boxIndex">
                <a href="#" target="_blank" class="iBlock relative">
                    <figure class="relative">
                        <img src="" alt="" />
                    </figure>
                    <em class="iBlock absolute fourIcon Hot"></em>
                    <p style="text-align: center;font-size: 14px;">美白嫩肤</p>
                </a>
            </div>
            <div class="itemlist boxIndex">
                <a href="#" target="_blank" class="iBlock relative">
                    <figure class="relative">
                        <img src="" alt="" />
                    </figure>
                    <em class="iBlock absolute fourIcon Hot"></em>
                    <p style="text-align: center;font-size: 14px;">美白嫩肤</p>
                </a>
            </div>
            <div class="itemlist boxIndex">
                <a href="#" target="_blank" class="iBlock relative">
                    <figure class="relative">
                        <img src="" alt="" />
                    </figure>
                    <p style="text-align: center;font-size: 14px;">美白嫩肤</p>
                </a>
            </div>
            <div class="itemlist boxIndex">
                <a href="#" target="_blank" class="iBlock relative">
                    <figure class="relative">
                        <img src="" alt="" />
                    </figure>
                    <p style="text-align: center;font-size: 14px;">美白嫩肤</p>
                </a>
            </div>
            <div class="itemlist boxIndex">
                <a href="#" target="_blank" class="iBlock relative">
                    <figure class="relative">
                        <img src="" alt="" />
                    </figure>
                    <p style="text-align: center;font-size: 14px;">美白嫩肤</p>
                </a>
            </div>
        </div>
    </section>

    <!-- 专家团队 -->
    <section class="zjBoxs wPub">
        <div class="zjTit clearfix">
            <h3 class="fl">专家团队</h3>
        </div>
        <div class="zjContent relative">
            <div class="zjlists">
                <div class="zjlist relative">
                    <img src="{{ asset('image') }}/1200.png" alt="" />
                    <div class="zjTxt absolute">
                        <h3>院长</h3>
                        <div class="zjzc">
                            姓名
                        </div>
                        <div class="zjbm">
                            <h5>描述</h5>
                        </div>
                        <a href="#" class="iBlock boxIndex">点击详情</a>
                    </div>
                </div>

                <div class="zjlist relative">
                    <img src="{{ asset('image') }}/1200.png" alt="" />
                    <div class="zjTxt absolute">
                        <h3>副院长</h3>
                        <div class="zjzc">
                            姓名
                        </div>
                        <div class="zjbm">
                            <h5>描述</h5>
                        </div>
                        <a href="#" class="iBlock boxIndex">点击详情</a>
                    </div>
                </div>
            </div>

            <div class="zjhd absolute">
                <div class="zjtabBd">
                    <li class="zjcicle relative"> <img style="width: 108px;height: 108px;" src="{{ asset('image') }}/150.png" /> <em
                            class="iBlock fourIcon absolute"></em> </li>
                    <li class="zjcicle relative"> <img style="width: 108px;height: 108px;" src="{{ asset('image') }}/150.png" /> <em
                            class="iBlock fourIcon absolute"></em> </li>
                </div>
            </div>
        </div>
    </section>

    <!-- 完美脱变 -->
    <section class="truealBoxs wPub">
        <a href="#"> 
            <img src="{{ asset('image') }}/150.png" class="after" />
            <img src="{{ asset('image') }}/1502.png" class="before" />
        </a>
        <a href="#"> 
            <img src="{{ asset('image') }}/150.png" class="after" />
            <img src="{{ asset('image') }}/1502.png" class="before" />
        </a>
        <a href="#"> 
            <img src="{{ asset('image') }}/150.png" class="after" />
            <img src="{{ asset('image') }}/1502.png" class="before" />
        </a>
        <a href="#"> 
            <img src="{{ asset('image') }}/150.png" class="after" />
            <img src="{{ asset('image') }}/1502.png" class="before" />
        </a>
        <a href="#"> 
            <img src="{{ asset('image') }}/150.png" class="after" />
            <img src="{{ asset('image') }}/1502.png" class="before" />
        </a>
        <a href="#"> 
            <img src="{{ asset('image') }}/150.png" class="after" />
            <img src="{{ asset('image') }}/1502.png" class="before" />
        </a>
        <a href="#"> 
            <img src="{{ asset('image') }}/150.png" class="after" />
            <img src="{{ asset('image') }}/1502.png" class="before" />
        </a>
        <a href="#"> 
            <img src="{{ asset('image') }}/150.png" class="after" />
            <img src="{{ asset('image') }}/1502.png" class="before" />
        </a>
        <a href="#"> 
            <img src="{{ asset('image') }}/150.png" class="after" />
            <img src="{{ asset('image') }}/1502.png" class="before" />
        </a>
        <a href="#"> 
            <img src="{{ asset('image') }}/150.png" class="after" />
            <img src="{{ asset('image') }}/1502.png" class="before" />
        </a> 
        <span class="iBlock fl">
            <img style="width: 600px;height: 150px;" src="{{ asset('image') }}/600.png" />
        </span>
        <a href="#"> 
            <img src="{{ asset('image') }}/150.png" class="after" />
            <img src="{{ asset('image') }}/1502.png" class="before" />
        </a>
        <a href="#"> 
            <img src="{{ asset('image') }}/150.png" class="after" />
            <img src="{{ asset('image') }}/1502.png" class="before" />
        </a>
        <a href="#"> 
            <img src="{{ asset('image') }}/150.png" class="after" />
            <img src="{{ asset('image') }}/1502.png" class="before" />
        </a>
        <a href="#"> 
            <img src="{{ asset('image') }}/150.png" class="after" />
            <img src="{{ asset('image') }}/1502.png" class="before" />
        </a>
        <a href="#"> 
            <img src="{{ asset('image') }}/150.png" class="after" />
            <img src="{{ asset('image') }}/1502.png" class="before" />
        </a>
        <a href="#"> 
            <img src="{{ asset('image') }}/150.png" class="after" />
            <img src="{{ asset('image') }}/1502.png" class="before" />
        </a>
        <a href="#"> 
            <img src="{{ asset('image') }}/150.png" class="after" />
            <img src="{{ asset('image') }}/1502.png" class="before" />
        </a>
        <a href="#"> 
            <img src="{{ asset('image') }}/150.png" class="after" />
            <img src="{{ asset('image') }}/1502.png" class="before" />
        </a>
        <a href="#"> 
            <img src="{{ asset('image') }}/150.png" class="after" />
            <img src="{{ asset('image') }}/1502.png" class="before" />
        </a>
        <a href="#"> 
            <img src="{{ asset('image') }}/150.png" class="after" />
            <img src="{{ asset('image') }}/1502.png" class="before" />
        </a>
        <a href="#"> 
            <img src="{{ asset('image') }}/150.png" class="after" />
            <img src="{{ asset('image') }}/1502.png" class="before" />
        </a>
    </section>

    <!-- 新闻资讯 -->
    <section class="glmppBoxs wPub">
        <div class="ppcontents clearfix">
            <div class="ppXushu ppcontent relative fl">
                <h3>新闻资讯</h3>
                <ul>
                    @foreach ($news as $vo)
                        <li class="boxIndex">
                            <a href="{{ $vo->link() }}">
                                <img src="{{ $vo->avatar }}" alt="{{ $vo->titile }}" style="width: 285px;height: 250px;">
                                <p style="font-size: 14px;text-indent: 2em;">
                                    {{ $vo->title }}
                                </p>
                            </a>
                        </li>                       
                    @endforeach
                </ul>
                <span class="ppBtn ppPrev absolute">
                    <i class="fourIcon iBlock"></i>
                </span>
                <span
                    class="ppBtn ppNext absolute"><i class="fourIcon iBlock"></i>
                </span>
            </div>
            <div class="ppSafe ppcontent relative fl">
                <h3>企业文化</h3>
                <ul>
                    @foreach ($company as $vo)
                        <li class="boxIndex">
                            <a href="">
                                <img src="{{ $vo->avatar }}" alt="{{ $vo->titile }}" style="width: 285px;height: 250px;">
                                <p style="font-size: 14px;text-indent: 2em;">
                                    {{ $vo->title }}
                                </p>
                            </a>
                        </li>                       
                    @endforeach
                </ul>
                <span class="ppBtn ppPrev absolute">
                    <i class="fourIcon iBlock"></i>
                </span>
                <span
                    class="ppBtn ppNext absolute"><i class="fourIcon iBlock"></i>
                </span>
            </div>
            <div class="ppshebei ppcontent relative fl">
                <h3>案例展示</h3>
                <ul>
                    @foreach ($list as $vo)
                        <li class="boxIndex">
                            <a href="">
                                <img src="{{ $vo->avatar }}" alt="{{ $vo->titile }}" style="width: 285px;height: 250px;">
                                <p style="font-size: 14px;text-indent: 2em;">
                                    {{ $vo->title }}
                                </p>
                            </a>
                        </li>                       
                    @endforeach
                </ul>
                <span class="ppBtn ppPrev absolute">
                    <i class="fourIcon iBlock"></i>
                </span>
                <span
                    class="ppBtn ppNext absolute"><i class="fourIcon iBlock"></i>
                </span>
            </div>
            <div class="ppEnvironment ppcontent relative fl">
                <h3>培训技术</h3>
                <ul>
                    @foreach ($training as $vo)
                        <li class="boxIndex">
                            <a href="">
                                <img src="{{ $vo->avatar }}" alt="{{ $vo->titile }}" style="width: 285px;height: 250px;">
                                <p style="font-size: 14px;text-indent: 2em;">
                                    {{ $vo->title }}
                                </p>
                            </a>
                        </li>                       
                    @endforeach
                </ul>
                <span class="ppBtn ppPrev absolute">
                    <i class="fourIcon iBlock"></i>
                </span>
                <span
                    class="ppBtn ppNext absolute"><i class="fourIcon iBlock"></i>
                </span>
            </div>
        </div>
    </section>
@endsection
